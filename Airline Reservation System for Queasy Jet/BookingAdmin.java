...
public class BookingAdmin
{
    ...
    /**
     * Constructor for objects of class BookingAdmin
     */
    public BookingAdmin(Aircraft plane){
        this.plane = plane;
        setupAdmin();
        menu();
    }
    ...
    private void setupAdmin(){
        for(int c = 0; c< 100; c++){
                String pfn = c+"FName";
                String pln = c+"LName";
                EconomyPassenger ep = new EconomyPassenger(pfn, pln,75, Economy.GLASGOW_BASE_FARE);
                ep.setLuggage(15.0);
    ...
    private void addEconomyPassenger(){
        ...
        switch (plane.getRoute()){
            case "Cape Town": 
            newPassenger = new EconomyPassenger(firstName, lastName, weight, Economy.CAPETOWN_BASE_FARE);
            break;
            case "Glasgow":
            newPassenger = new EconomyPassenger(firstName, lastName, weight, Economy.GLASGOW_BASE_FARE);
            break;
            default:
            return;
        }
